...
public class FilteringXKOL implements FilteringKOL {
	...
	public void loadCheckpoint() {
		Type memType = new TypeToken<List<Integer>>() {
		}.getType();
	...
	public Map<String, Integer> filteredKOLMap(Map<String, Integer> kolMap, int maxFilteredKOL) {
		return kolMap.entrySet().stream().sorted((e1, e2) -> Integer.compare(e2.getValue(), e1.getValue()))
				.limit(maxFilteredKOL).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue,
	...
	public void runTask() {
		...
		setType = new TypeToken<LinkedHashMap<String, Integer>>() {
		}.getType();
		...
		LinkedHashMap<String, Integer> topLinks = (LinkedHashMap<String, Integer>) filteredKOLMap(linkFollowerMap,
				maxFilteredKOL);
